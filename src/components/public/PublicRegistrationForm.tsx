...
import { formatCurrency, formatDate, isValidGhanaPhone, formatGhanaPhone } from '@/lib/utils';
import type { 
...
const createRegistrationSchema = (customFields: CustomField[]) => {
  ...
  customFields.forEach(field => {
    ...
    if (field.is_required) {
      fieldSchema = field.field_type === 'checkbox' 
...
export const PublicRegistrationForm: React.FC<PublicRegistrationFormProps> = ({
  ...
  const districts = Array.from(new Set(churches.map((c) => c.district).filter(Boolean)));
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          if (checked) {
                            ...
                            formField.onChange(currentValues.filter(val => val !== option));
                          }
                        }}
                      />
      ...
      <Card>
        <CardHeader>
          <div className="text-center space-y-4">
            <CardTitle className="text-2xl font-bold">{camp.name}</CardTitle>
            <div
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
              ...
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-sm">From {formatCurrency(camp.base_fee)}</span>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-amber-800">
                  Registration closes on {formatDate(camp.registration_deadline)}
                </span>
              </div>
            </div>
          </div>
